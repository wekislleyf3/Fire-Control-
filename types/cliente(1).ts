...
export type Cliente = {
  ...
  /** Etapa atual no pipeline operacional (Método Fire). Ver types/pipeline.ts. */
  etapa_pipeline: import("./pipeline").EtapaPipeline;
  created_at: string;
};

/** Campos aceitos ao criar/editar um cliente pelo formulário. Matrícula é sempre gerada pelo banco. */
export type ClienteInput = Omit<Cliente, "id" | "created_at" | "matricula" | "etapa_pipeline"> & {
  etapa_pipeline?: import("./pipeline").EtapaPipeline;
};
