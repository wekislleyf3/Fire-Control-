...
async function emitirOuReaproveitar(
  ...
  if (insertError || !novaEmissao) {
    // Nunca repassa insertError.message pro cliente: mensagens de erro do
    // Postgres/Supabase podem citar nome de coluna, constraint, tabela etc.
    // — informação de schema que não deveria sair do servidor. O detalhe
    // completo vai só pro log do servidor; quem chamou a API recebe uma
    // mensagem genérica.
    console.error("[laudos/emitir] falha ao inserir selo:", insertError);
    return NextResponse.json({ error: "Não foi possível emitir o selo. Tente novamente." }, { status: 500 });
  }
