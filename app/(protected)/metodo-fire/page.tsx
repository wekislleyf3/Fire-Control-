...
import { procedimentosService, ValidationError, type ItemForm } from "@/lib/services/procedimentosService";
...
export default function MetodoFirePage() {
  ...
  async function carregar() {
    setLoading(true);
    const mapa = await procedimentosService.listMapaPorTipo(supabase);
  ...
  async function abrirTipo(tipo: string) {
    ...
    if (existente) {
      ...
      const data = await procedimentosService.listItens(supabase, existente.id);
      setItens(
        data.map((i) => ({
          id: i.id,
          chave: i.chave,
  ...
  async function salvar() {
    ...
    try {
      await procedimentosService.salvar(supabase, {
        tipoEquipamento: tipoAberto,
        existente: procedimentos[tipoAberto],
        form,
        itens,
      });
      ...
      const mensagem = err instanceof ValidationError ? err.message : `Erro ao salvar: ${(err as Error).message}`;
      setError(mensagem);
    } finally {
      setSalvando(false);
    }
  ...
  async function restaurarPadrao(tipo: string) {
    ...
    setSalvando(true);
    try {
      await procedimentosService.restaurarPadrao(supabase, existente.id);
      setSucesso(`"${tipo}" voltou a usar o checklist padrão do sistema.`);
      await carregar();
      fechar();
    } catch (err) {
      setError(`Erro ao restaurar padrão: ${(err as Error).message}`);
    } finally {
      setSalvando(false);
    }
  }
