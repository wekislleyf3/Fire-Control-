...
async function enviarArquivo(clienteId: string, file: File) {
  const caminho = `documentos/${clienteId}/${Date.now()}-${file.name}`;
  const { error: uploadError } = await supabase.storage.from("firecontrol-files").upload(caminho, file);
...
export default function DocumentosPage() {
  ...
  async function loadDocumentos(clId: string) {
    ...
    let query = supabase.from("documentos").select("*").eq("cliente_id", clId).order("created_at", { ascending: false });
    if (!mostrarHistorico) query = query.eq("status", "vigente");

    const { data, error: err } = await query;
    if (err) setError(`Erro ao carregar documentos: ${err.message}`);
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      const { url, nome } = await enviarArquivo(clienteId, file);
      ...
      setError(`Erro ao enviar documento: ${(err as Error).message}`);
    } finally {
      setUploading(false);
    }
  }
  ...
  async function salvarValidade(doc: Documento) {
    ...
    const { error: err } = await supabase
      .from("documentos")
      .update({ validade: novaValidade || null })
      .eq("id", doc.id);
  ...
  async function confirmarRenovacao(doc: Documento) {
    ...
    try {
      const { url, nome } = await enviarArquivo(clienteId, arquivoRenovacao);
      ...
      if (insertError || !novoDoc) throw new Error(insertError?.message ?? "erro ao criar novo documento");

      const { error: updateError } = await supabase
        .from("documentos")
        .update({ status: "substituido", substituido_por: novoDoc.id })
        .eq("id", doc.id);
      ...
      setError(`Erro ao renovar documento: ${(err as Error).message}`);
    } finally {
