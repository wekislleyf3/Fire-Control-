...
import { agendaService, ValidationError } from "@/lib/services/agendaService";
...
export default function AgendaPage() {
  ...
  async function carregarEventos() {
    ...
    try {
      const data = await agendaService.listPorPeriodo(supabase, inicioStr, fimStr);
      setEventos(data);
    } catch (err) {
      setError(`Erro ao carregar agenda: ${(err as Error).message}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    async function carregarBase() {
      const [cl, eq] = await Promise.all([clientesService.list(supabase), equipamentosService.list(supabase)]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await agendaService.create(supabase, form);
      setShowForm(false);
      setForm({ ...emptyForm });
      carregarEventos();
    } catch (err) {
      const mensagem =
        err instanceof ValidationError ? err.message : `Erro ao salvar evento: ${(err as Error).message}`;
      setError(mensagem);
    } finally {
      setSalvando(false);
    }
  ...
  async function atualizarStatus(evento: EventoAgenda, status: "concluido" | "cancelado") {
    try {
      await agendaService.atualizarStatus(supabase, evento.id, status);
      carregarEventos();
    } catch (err) {
      setError(`Erro ao atualizar evento: ${(err as Error).message}`);
    }
  }
