    ...
    <div>
      ...
      <div className="sm:hidden space-y-3">
        ...
        {!loading && clientesFiltrados.length === 0 && (
          <p className="text-center text-brand-slate/60 py-6">Nenhum cliente encontrado.</p>
        )}
        {clientesFiltrados.map((c) => (
          <div key={c.id} className="bg-white border border-black/5 rounded-lg p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-brand-slate/60 mt-0.5">
                  {c.matricula ?? "—"} · {c.cidade}
                  {c.estado ? `/${c.estado}` : ""} · {c.telefone || "sem telefone"}
                </p>
              </div>
              <span className="text-xs px-2 py-0.5 rounded-full bg-brand-fog text-brand-slate shrink-0">
                {c.tipo_pessoa === "fisica" ? "PF" : "PJ"}
              </span>
            </div>

            <div className="flex flex-wrap items-center gap-2 mt-3">
              <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">{labelEtapa(c.etapa_pipeline)}</span>
              <span className="text-xs capitalize text-brand-slate/70">{c.status}</span>
            </div>

            <div className="flex flex-wrap gap-2 mt-3">
              <Link href={`/clientes/${c.id}`} className="text-xs px-2 py-1 rounded border">
                Ver cliente
              </Link>
              <button onClick={() => startEdit(c)} className="text-xs px-2 py-1 rounded border">
                Editar
              </button>
              {deletingId === c.id ? (
                <>
                  <button onClick={() => handleDelete(c.id)} className="text-xs px-2 py-1 rounded bg-brand-red text-white">
                    Confirmar exclusão
                  </button>
                ...
                <button onClick={() => setDeletingId(c.id)} className="text-xs px-2 py-1 rounded border text-brand-red">
                  Excluir
                </button>
              )}
            </div>
