...
import { calcularUrgencia, severityStyle, hojeBrasiliaStr } from "@/lib/alerts";
...
async function getClienteDetalhe(id: string) {
  ...
  const [clienteRes, equipamentosRes, documentosRes, inspecoesRes, pipelineHistRes, laudosRes, agendaRes, proximaVisitaRes] =
    await Promise.all([
  ...
  for (const insp of inspecoes) {
    timeline.push({
      ...
      tipo: "inspecao",
      titulo: `Inspeção em ${insp.equipamentos?.codigo_interno ?? "equipamento"}`,
      detalhe: insp.resultado === "conforme" ? "Conforme" : "Não conforme",
    });
  }

  for (const laudo of laudosRes.data ?? []) {
    timeline.push({ data: laudo.data_emissao, tipo: "diagnostico", titulo: "Diagnóstico emitido" });
  }

  for (const hist of pipelineHistRes.data ?? []) {
    timeline.push({
  ...
  for (const ev of agendaRes.data ?? []) {
  ...
  const ultimoDiagnosticoData = laudosRes.data?.[0]?.data_emissao ?? null;
  ...
  return {
    ...
    proximosVencimentos,
    timeline,
    proximaVisita: proximaVisitaRes.data ?? null,
    ultimoRelatorio,
  };
}
