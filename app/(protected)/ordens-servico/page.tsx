...
import { ordensServicoService } from "@/lib/services/ordensServicoService";
...
import { hojeBrasiliaStr } from "@/lib/alerts";
...
export default function OrdensServicoPage() {
  ...
  async function carregarTudo() {
    ...
    try {
      const [cl, eq, ev, os] = await Promise.all([
        ...
        agendaRepository.listAgendados(supabase),
  ...
  async function confirmarConclusao() {
    ...
    setError(null);
    try {
      const assinaturaBlob = await (await fetch(assinaturaDataUrl)).blob();
      await ordensServicoService.concluir(supabase, osConcluindo, {
        assinaturaBlob,
        nomeAssinante,
      });
      ...
      setError(`Erro ao concluir OS: ${(err as Error).message}`);
    } finally {
      setConcluindo(false);
    }
  }
