...
import { calcularUrgencia, calcularStatusEquipamento, hojeBrasilia, hojeBrasiliaStr } from "@/lib/alerts";
...
async function getOperacao() {
  ...
  const [equipamentos, documentos, clientes, inspecoesHoje, laudosHoje, agendaHojeBruta, agendaProximaBruta] =
    await Promise.all([
      equipamentosRepository.list(supabase),
      documentosRepository.listTodosComValidade(supabase),
      clientesRepository.list(supabase),
      inspecoesRepository.listDesde(supabase, hojeISO),
      laudosRepository.listDesde(supabase, hojeISO),
      agendaRepository.listHoje(supabase, hojeStr),
      agendaRepository.listProximosAgendados(supabase, hojeStr, 5),
    ]);

  const nomePorCliente = new Map(clientes.map((c) => [c.id, c.razao_social]));
  const totalClientes = clientes.length;

  // A página exibe ev.clientes?.razao_social (antes vinha de um join) —
  // mantemos esse mesmo formato aqui pra não precisar mexer no JSX abaixo.
  const enriquecerComCliente = <T extends { cliente_id: string | null }>(eventos: T[]) =>
    eventos.map((ev) => ({ ...ev, clientes: { razao_social: ev.cliente_id ? nomePorCliente.get(ev.cliente_id) ?? null : null } }));
  ...
  for (const eq of equipamentos) {
    if (calcularStatusEquipamento(eq) === "atencao") equipAtencao++;
  ...
  const docsVencidos = documentos.filter((d) => {
    const v = new Date((d.validade as string) + "T00:00:00-03:00");
  ...
  return {
    ...
    agendaHoje: enriquecerComCliente(agendaHojeBruta),
    agendaProxima: enriquecerComCliente(agendaProximaBruta),
  };
}
