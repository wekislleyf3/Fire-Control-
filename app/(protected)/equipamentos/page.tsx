...
import type { EquipamentoInput } from "@/types/equipamento";
import { equipamentosService, ValidationError } from "@/lib/services/equipamentosService";
...
import { TIPOS_EQUIPAMENTO as TIPOS } from "@/lib/metodoFire";
import { calcularStatusEquipamento } from "@/lib/alerts";
...
export default function EquipamentosPage() {
  ...
  const [gerandoEtiquetaId, setGerandoEtiquetaId] = useState<string | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    const [eq, cl] = await Promise.all([equipamentosService.list(supabase), clientesService.list(supabase)]);
    setEquipamentos(eq);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editingId) {
        await equipamentosService.update(supabase, editingId, form as unknown as EquipamentoInput);
      } else {
        await equipamentosService.create(supabase, form as unknown as EquipamentoInput);
      }
      cancelForm();
      loadData();
      router.refresh();
    } catch (err) {
      const mensagem = err instanceof ValidationError ? err.message : `Erro ao salvar equipamento: ${(err as Error).message}`;
      setError(mensagem);
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(id: string) {
    setError(null);
    try {
      await equipamentosService.remove(supabase, id);
      loadData();
      router.refresh();
    } catch (err) {
      setError(`Erro ao excluir equipamento: ${(err as Error).message}`);
    } finally {
      setDeletingId(null);
    }
  }
  ...
  async function handlePhotoUpload(eq: Equipamento, file: File) {
    ...
    setError(null);
    try {
      await equipamentosService.atualizarFoto(supabase, eq, file);
      loadData();
    } catch (err) {
      setError(`Erro ao enviar foto: ${(err as Error).message}`);
    } finally {
      setUploadingId(null);
    }
  }

  async function handleGerarEtiqueta(eq: Equipamento, tamanho: TamanhoEtiqueta) {
    setGerandoEtiquetaId(eq.id);
    try {
      await gerarEtiquetaEquipamento(eq, tamanho);
    } catch (err) {
      setError(`Erro ao gerar etiqueta: ${(err as Error).message}`);
    } finally {
    ...
    <div>
      ...
      {showForm && (
        ...
        >
          ...
          <div className="col-span-1 sm:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-black/5 pt-4">
            <div>
      ...
      {/* Celular: lista de cards (a tabela completa fica reservada pra telas maiores) */}
      <div className="sm:hidden space-y-3">
        {loading && <p className="text-center text-brand-slate/60 py-6">Carregando...</p>}
        {!loading && equipamentosFiltrados.length === 0 && (
          <p className="text-center text-brand-slate/60 py-6">
            {busca.trim() ? "Nenhum equipamento encontrado para essa busca." : "Nenhum equipamento cadastrado ainda."}
          </p>
        )}
        {equipamentosFiltrados.map((eq) => (
          <div key={eq.id} className="bg-white border border-black/5 rounded-lg p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                <Link href={`/equipamentos/${eq.id}`} className="text-brand-red font-medium hover:underline">
                  {eq.codigo_interno}
                </Link>
                <p className="text-xs text-brand-slate/60 mt-0.5">
                  {eq.tipo} · {eq.localizacao || "sem localização"}
                </p>
              </div>
              <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${statusColor[calcularStatusEquipamento(eq)]}`}>
                {calcularStatusEquipamento(eq)}
              </span>
            </div>

            <div className="flex flex-wrap gap-2 mt-3">
              <Link href={`/equipamentos/${eq.id}`} className="text-xs px-2 py-1 rounded border">
                Ver detalhes
              </Link>
              <button onClick={() => startEdit(eq)} className="text-xs px-2 py-1 rounded border">
                Editar
              </button>
              {(["pequena", "media", "grande"] as const).map((tam) => (
                <button
                  key={tam}
                  onClick={() => handleGerarEtiqueta(eq, tam)}
                  disabled={gerandoEtiquetaId === eq.id}
                  className="text-xs px-2 py-1 rounded border disabled:opacity-50"
                >
                  Etiqueta {tam === "pequena" ? "P" : tam === "media" ? "M" : "G"}
                </button>
              ))}
              {deletingId === eq.id ? (
                <>
                  <button onClick={() => handleDelete(eq.id)} className="text-xs px-2 py-1 rounded bg-brand-red text-white">
                    Confirmar exclusão
                  </button>
                ...
                <button onClick={() => setDeletingId(eq.id)} className="text-xs px-2 py-1 rounded border text-brand-red">
                  Excluir
                </button>
              )}
            </div>
      ...
      {/* Tablet/desktop: tabela completa, com todas as ações (foto, QR inline etc) */}
            ...
            {equipamentosFiltrados.map((eq) => (
              <tr key={eq.id} className="border-t border-black/5">
                <td className="px-4 py-3 font-medium">
                  <Link href={`/equipamentos/${eq.id}`} className="text-brand-red hover:underline">
                    {eq.codigo_interno}
                  </Link>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs ${statusColor[calcularStatusEquipamento(eq)]}`}>
                    {calcularStatusEquipamento(eq)}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-1 items-start">
                    ...
                    >
                      {qrFor === eq.id ? "ocultar QR" : "ver QR"}
                    </button>
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-brand-slate/50">Etiqueta:</span>
                      {(["pequena", "media", "grande"] as const).map((tam) => (
                        <button
                          key={tam}
                          onClick={() => handleGerarEtiqueta(eq, tam)}
                          disabled={gerandoEtiquetaId === eq.id}
                          className="text-[10px] px-1.5 py-0.5 rounded border text-brand-slate disabled:opacity-50 hover:bg-brand-fog"
                          title={`Etiqueta ${tam}`}
                        >
                          {tam === "pequena" ? "P" : tam === "media" ? "M" : "G"}
                        </button>
                      ))}
                    </div>
                  </div>
                  {qrFor === eq.id && (
                    <div className="mt-2">
                      <QRCodeSVG
                        value={`${typeof window !== "undefined" ? window.location.origin : ""}/equipamentos/${eq.id}`}
                        size={80}
                      />
                      <p className="text-[10px] text-brand-slate/50 mt-1 max-w-[80px]">
                        Aponta para a página deste equipamento
                      </p>
                    </div>
                  )}
                </td>
