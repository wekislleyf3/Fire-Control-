...
import { calcularStatusEquipamento, hojeBrasiliaStr } from "@/lib/alerts";
...
export default function EquipamentoDetalhePage() {
  ...
  const [dataEvento, setDataEvento] = useState(hojeBrasiliaStr());
  ...
  async function carregar() {
    ...
    setError(null);
    try {
      const detalhe = await equipamentosService.carregarDetalhe(supabase, params.id);
      if (!detalhe) {
        setNaoEncontrado(true);
        return;
      }
      setEquipamento(detalhe.equipamento);
      setCliente(detalhe.cliente);
      setInspecoes(detalhe.inspecoes);
      setHistorico(detalhe.historico as EventoHistorico[]);
      setDocumentos(detalhe.documentos);
      setOrdensServico(detalhe.ordensServico);
      setPdfsEmitidos(detalhe.pdfsEmitidos);
    } catch (err) {
      setError(`Erro ao carregar equipamento: ${(err as Error).message}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleFotoUpload(file: File) {
    ...
    try {
      await equipamentosService.atualizarFoto(supabase, equipamento, file);
      carregar();
    } catch (err) {
  ...
  async function handleRegistrarEvento(e: React.FormEvent) {
    ...
    try {
      await equipamentosService.registrarEventoManual(supabase, {
      ...
      setEventoCustom("");
      setDataEvento(hojeBrasiliaStr());
      setObservacaoEvento("");
      carregar();
    } catch (err) {
      const mensagem =
        err instanceof ValidationError ? err.message : `Erro ao registrar evento: ${(err as Error).message}`;
      setError(mensagem);
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-4 gap-3">
        ...
        <span
          className={`px-3 py-1 rounded-full text-xs font-medium shrink-0 ${statusColor[calcularStatusEquipamento(equipamento)]}`}
        >
          {calcularStatusEquipamento(equipamento)}
        </span>
      </div>
