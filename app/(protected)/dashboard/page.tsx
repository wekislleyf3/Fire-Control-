...
async function getDashboardData() {
  ...
  const [clientesAtivos, totalClientes, equipamentos, clientes, inspecoesRecentes] = await Promise.all([
    clientesRepository.count(supabase, { apenasAtivos: true }),
    clientesRepository.count(supabase),
    // Versões enxutas: o dashboard só precisa de algumas colunas, não da
    // linha inteira de cada cliente/equipamento (ver repositórios).
    equipamentosRepository.listResumoDashboard(supabase),
    clientesRepository.listNomes(supabase),
    inspecoesRepository.listRecentes(supabase, 6),
  ]);
  ...
  return {
    ...
    ultimasInspecoes: inspecoesRecentes.map((i) => ({
      ...i,
      equipamentos: { codigo_interno: codigoPorEquipamento.get(i.equipamento_id) ?? null },
      clientes: { razao_social: nomePorCliente.get(i.cliente_id) ?? null },
    })),
  };
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-base text-brand-slate/70">Visão geral da operação, atualizada em tempo real.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
        {cards.map((c) => (
          ...
          >
            ...
            <p className="text-xs text-brand-slate/50 mt-1">{c.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Gráfico */}
        <div className="lg:col-span-3 bg-white border border-black/5 rounded-xl p-5 md:p-6">
          <p className="font-display text-xl mb-3">Equipamentos por tipo</p>
          <EquipamentosPorTipoChart data={d.porTipo} />
        </div>

        {/* Próximos vencimentos */}
        <div className="lg:col-span-2 bg-white border border-black/5 rounded-xl p-5 md:p-6">
          <div className="flex items-center justify-between mb-3">
            <p className="font-display text-xl">Próximos vencimentos</p>
      ...
      {/* Atividade recente */}
      <div className="bg-white border border-black/5 rounded-xl p-5 md:p-6">
        <p className="font-display text-xl mb-3">Últimas inspeções</p>
