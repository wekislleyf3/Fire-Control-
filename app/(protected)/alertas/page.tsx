...
async function getAlertas(): Promise<LinhaAlerta[]> {
  ...
  for (const eq of equipamentos) {
    const clienteNome = nomePorCliente.get(eq.cliente_id) ?? "—";
    ...
    <div>
      ...
      <div className="bg-white border border-black/5 rounded-lg overflow-hidden sm:overflow-x-auto">
        ...
        <div className="sm:hidden divide-y divide-black/5">
          {alertas.length === 0 && (
            <p className="px-4 py-6 text-center text-brand-slate/60">Nenhum vencimento nos próximos 90 dias. Tudo em dia.</p>
          )}
          {alertas.map((a, i) => (
            <div key={`${a.equipamento.id}-${a.tipoData}-${i}`} className="px-4 py-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="font-medium text-sm">{a.equipamento.codigo_interno}</p>
                  <p className="text-xs text-brand-slate/60">
                    {a.clienteNome} · {a.tipoData}
                  </p>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full border shrink-0 ${severityStyle[a.severity]}`}>
                  {a.label}
                </span>
              </div>
              <p className="text-xs text-brand-slate/60 mt-1">{new Date(a.data + "T00:00:00").toLocaleDateString("pt-BR")}</p>
            </div>
          ))}
        </div>

        {/* Tablet/desktop */}
        <table className="w-full text-sm min-w-[700px] hidden sm:table">
