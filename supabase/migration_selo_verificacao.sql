-- DEPRECADO — substituído por migration_laudos_autenticacao.sql.
-- lib/selo.ts e app/api/selo/route.ts, citados abaixo, não existem mais no
-- código; o sistema de selo atual usa a tabela `laudos_autenticacao`. Este
-- arquivo fica só como histórico de quem já rodou esta migration em produção
-- (a coluna `codigo_verificacao` que ela cria não é mais lida por nada).
-- Não use como referência para o mecanismo de verificação atual.
--
